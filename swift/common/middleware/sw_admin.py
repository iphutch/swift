...
class SWAdminMiddleware(object):
    ...
    Admin/Op use only :
    If the path is /sw_admin, and  'enable_sw_admin' in the proxy-server.conf
    is set to 'false' it will respond with 412 "Bad URL" as the body.

    To invalidate/delete cached auth tokens, set 'enable_sw_admin' to 'true'
    ...
    def delete_cached_token(self, user_id):
        ...
        result1 = self.memcache.delete('%s/user/%s' %
                                       (self.reseller_prefixes[0], user_id))
